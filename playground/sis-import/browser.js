...
import {status, text} from '../../src/helpers/fetch-helpers'

const COURSES_URL = 'https://www.stolaf.edu/sis/st-courses.cfm'

const fetch = (url, args={}) => global.fetch(url, {cache: 'no-cache', credentials: 'same-origin', mode: 'same-origin', ...args}).catch(catchAndThrowNetworkError)
...
function catchAndThrowNetworkError(err) {
...
function getCoursesFromHtml(html, term) {
	...
	return Array.from(courseRows).map(row => convertToCourse(term, row))
}
...
function buildStudentFromCourses(courses) {

}


function logCourses(courses) {
	let stringified = map(courses, c => `${c.term} ${c.deptnum} ${(c.lab ? 'L' : '')}`).join('\n')
