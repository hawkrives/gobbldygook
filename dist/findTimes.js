...
var cleanTimestringSegment = _interopRequire(require("./cleanTimestringSegment"));
...
function findTimes(timestring) {
  var cleanedTimestring = timestring.replace(/:/g, ""); // 8:00-9:25 => 800-925

  var endsInPM = false;
  var startsInAM = false;

  // Split the string apart and clean it up.
  var split = cleanedTimestring.split("-");
  var start = cleanTimestringSegment(split[0]);
  var end = cleanTimestringSegment(split[1]);
  ...
  var amPmRegex = /([AP])\.?M\.?/i;
  var am = amPmRegex.exec(start);
  var pm = amPmRegex.exec(end);
