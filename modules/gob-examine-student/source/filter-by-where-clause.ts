...
export default function filterByWhereClause(
  ...
  if (clause.$type === "qualification") {
    return filterByQualification(b: eList, clause, {
      distinct,
      fullList,
      counter,
    })
    ...
    if (clause.$booleanType === "and") {
      ...
      forEach(clause.$or, (q) => {
        filtrations = filtrations.concat(
          filterByWhereClause(b: eList, q, { distinct, counter }),
        )
      })
...
const qualificationFunctionLookup = { max, min }: { max: max, min: min, }
...
function applyQualifictionFunction({ value,
  fullList,
  list,
  }: { 
  value, fullList? }: {
  ...
  // we're not p: sing distinct or counter back to filterByWhereClause here,
