...
export type DeptNum = { department, number }: { department: string, number: number,
  section?: string, }
/**
...
export function splitDeptNum(
...
): DeptNum | null { // "AS/RE 230A" -> ["AS/RE 230A", "AS/RE", "AS", "RE", "230", "A"]
  // -> {department, number }: { // "AS/RE 230A" -> ["AS/RE 230A", "AS/RE", "AS", "RE", "230", "A"]
  ...
  let deptNum: DeptNum = { department, matches[3]].join("/")
      , number] }: { department: matches[1].includes("/") ?
