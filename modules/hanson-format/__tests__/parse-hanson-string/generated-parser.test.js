...
describe('parse-hanson-string', () => {
	it('should not throw', () => {
		...
		r('seven of (CHEM 252, 260, 298, 379, (382 & 378), 384, 386, 388, 391, 398)')
		r('!AMCON 201')
		r('MATH 282.*.2014.1 | 244 | 252')
		r('one-point-five credits from (Ballet, Improvisation, International and Social, Modern)')
		...
		r('one of (MATH 230, 242,  236, 262, 266, (STAT 212 & 214 & 272), STAT 316, MATH 330)')
		...
		r('three courses from ( ChildRequirementNameHere, OrMaybeOtherChild ) where { level = 300 }')

		t('three departments from courses where { x = y }', 'cannot use a modifier with "departments"')
		t('one department from (Child) where { x = y }', 'must use "courses from" with "children where"')
		t('at most one credit from filter', 'can only use at-least style counters with non-course requests')
		t('at most one department from filter', 'can only use at-least style counters with non-course requests')
		...
		r('CSCI 121IL.Z')
		t('CSCI 121LI.Z')
		t('CSCI 121.a', 'A course section must be either an uppercase letter [A-Z] or an asterisk [*].')
		r('CSCI 121.*')
		r('CSCI 121.*.2014')
		t('CSCI 121.*.111', 'A course year must be either a four-digit year [e.g. 1994] or an asterisk [*].')
		r('CSCI 121.*.*')
		r('CSCI 121.*.*.1')
		r('CSCI 121.*.*.5')
		t('CSCI 121.*.*.0', 'A course semester must be either a number between 1 [Fall] and 5 [Summer Session 2], or an asterisk [*].')
		t('CSCI 121.*.*.6', 'A course semester must be either a number between 1 [Fall] and 5 [Summer Session 2], or an asterisk [*].')

		t('seven of (CSCI 121, CSCI 211)', 'you requested 7 items, but only gave 2 options')
	})
})
