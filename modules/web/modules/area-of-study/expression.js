...
import humanizeOperator from '../../../examine-student/humanize-operator'
...
function makeModifierExpression({ expr }) {
    const needs = `${humanizeOperator(expr.$count.$operator)} ${expr.$count
        .$num} ${plur(expr.$what, expr.$count.$num)}`
...
export function makeWhereExpression({ expr }) {
    ...
    const description = `${expr._counted} of ${needs} ${expr.$distinct
        ? 'distinct'
        : ''} ${expr.$count.$num === 1
        ? 'course'
        : 'courses'} from courses where ${qualifier}`
...
function makeOccurrenceExpression({ expr }) {
    const description = `${expr._counted} of ${humanizeOperator(
        expr.$count.$operator
    )} ${expr.$count.$num} ${expr.$count.$num === 1
        ? 'occurrence'
        : 'occurrences'} of `
