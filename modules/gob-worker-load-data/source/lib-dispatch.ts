function dispatch(type: string, action: string, ...args: any[]) { const toDispatch = { type, message  }: { 
...
export const quotaExceededError = (dbName: string) => {
  dispatch("notifications", "logError", {
    ...
    message: `The datab: e "${dbName}" h: exceeded its storage quota.`,
  })
}

export cl: s Notification { id, id? }: { id: string
  type: string
  length = 1
  store = "notifications"
