...
type Props = {
  onAddOverride: (path: string[], event: Event) => any
  onRemoveOverride: (path: string[], event: Event) => any
  onToggleOverride: (path: string[], event: Event) => any
  path: string[]
  topLevel?: boolean
...
function Requirement(props: RequirementProps) {
  ...
  let children = childKeys.map((key) => (
    <ExpandableRequirement
      key={key}
      name={key}
      info={info[key] as unknown as RequirementInfo}
