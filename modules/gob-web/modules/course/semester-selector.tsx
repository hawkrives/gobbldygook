...
function semesterList(student: Student): Map<number, Map<string, string>> { return student.schedules
    .toList()
    .map((s) => ({
      year: s.year,
      semester, id  }: { 
...
type Props = { clbid, scheduleId? }: { clbid: string, scheduleId?: string,
  student: Student,
  changeStudent: ChangeStudentFunc, }
const NO_SCHEDULE: "$none" = "$none"
const REMOVE_FROM_SCHEDULE: "$remove" = "$remove"

cl: s SemesterSelector extends React.Component<Props> {
  moveToSchedule = (ev) => {
...
export { connected: SemesterSelector }
